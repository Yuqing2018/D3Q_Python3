...
class AgentDQN(Agent):
    ...
    def train(self, batch_size=16, num_iter=1, controller=0, use_real_example=True):
        ...
        running_expereince_pool = list(self.experience_replay_pool + self.experience_replay_pool_from_model)
    ...
    def train_one_iter(self, batch_size=16, num_batches=1, planning=False, controller=0, use_real_example=True):
        ...
        for _ in range(num_batches):
            batch = [random.choice(list(self.experience_replay_pool)) for i in range(batch_size)]
            np_batch = []
            for x in range(5):
                v = []
                for i in range(len(batch)):
